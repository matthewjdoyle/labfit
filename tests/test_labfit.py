...
import labfit
from labfit import DataSeries, Dataset, Fitter, FitResult, Plotter, quick_fit, fit, fit_to_model, plot_fit, plot_residuals


def test_public_api_exports():
    expected = {
        'fit', 'quick_fit', 'fit_to_model', 'plot_fit', 'plot_residuals',
        'DataSeries', 'Dataset', 'Series', 'Fitter', 'FitResult', 'Plotter',
        'AsymmetricError',
    }
...
def test_csv_input_plot_and_residuals(tmp_path: Path):
    ...
    result = fit_to_model(csv, model='linear')
...
def test_custom_callable_model_and_plotter():
    ...
    fitter = Fitter(model=damped_oscillator, p0={'amplitude': 1.0, 'damping': 0.2, 'frequency': 1.0, 'phase': 0.0})
