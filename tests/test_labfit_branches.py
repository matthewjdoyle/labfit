...
from labfit.fitter_impl import _coerce_bounds, _coerce_series_input, _fit_single, _initial_guess, _normalize_sigma, fit_multi
from labfit.plot import _add_result_axes, _as_results, plot_result
from labfit.utils import as_array, effective_sigma, load_csv, propagate_errors
...
def test_newer_model_shapes_and_param_names():
    ...
    assert m.model_param_names('bimodal_gaussian') == ('amplitude1', 'mean1', 'sigma1', 'amplitude2', 'mean2', 'sigma2')
...
def test_newer_model_end_to_end_fits():
    ...
    res = fit(xd, yd, model='double_exponential', p0={'amplitude1': 2.0, 'tau1': 0.4, 'amplitude2': 1.0, 'tau2': 2.0})
    ...
    res = fit(xg, yg, model='gaussian_baseline', p0={'amplitude': 2.0, 'mean': 0.0, 'sigma': 1.0, 'm': 0.0, 'b': 0.0})
...
def test_types_utils_and_dataset_helpers(tmp_path: Path):
    ...
    x, y, sigma, sigma_low, sigma_high = load_csv(csv)
    assert len(x) == 2 and len(y) == 2
    series2 = _coerce_series_input(csv)
    assert np.allclose(series2.x, x)
    assert np.allclose(series2.y, y)
    assert np.allclose(series2.sigma, sigma)
    assert np.allclose(series2.sigma_low, sigma_low)
    assert np.allclose(series2.sigma_high, sigma_high)
...
def test_fitter_helpers_and_branches():
    ...
    res2 = fit(x, y, model='linear', ysigma=0.1)
    ...
    res4 = fit(x, y2, model=custom, p0={'amplitude': 1.0, 'damping': 0.2, 'frequency': 1.0, 'phase': 0.0}, bounds={'amplitude': (0.0, None)})
...
def test_private_branch_helpers_and_error_paths():
    ...
    assert as_array([1, 2]).dtype.kind == 'f'
