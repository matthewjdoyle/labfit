...
def fig_residuals():
    ...
    for xi, yi in zip(x, y):
        y_on_line = m * xi + c
        ax.plot([xi, xi], [yi, y_on_line], color="#7f8c8d", linewidth=1.0,
                linestyle="--", zorder=1)
...
def fig_chi2_surface():
    ...
    ax.plot(c_best, m_best, "*", markersize=12, color=OKABE[1], zorder=5,
            label="Best fit $(m^*, c^*)$")
    ...
    ax.annotate(
        ...
        fontsize=9.5, color=OKABE[1], ha="center",
...
def fig_optimizer_path():
    ...
    # Soft heatmap
    ax.contourf(ints, slopes, SS, levels=30, cmap="YlOrRd",
                alpha=0.12, zorder=0)
    # Reference contours
    cs_lines = ax.contour(ints, slopes, SS, levels=10, cmap="Greys",
    ...
    for k in range(1, n_arrows + 1):
        ...
        ax.arrow(raw_cs[i], raw_ms[i], dc * 0.7, dm * 0.7,
                 head_width=0.06, head_length=0.08,
                 fc=OKABE[3], ec=OKABE[3], linewidth=0, zorder=5)

    # Start and end markers
    ax.scatter([c0], [m0], color=OKABE[2], s=70, zorder=6, marker="s",
               edgecolors="white", linewidth=1.2)
    ax.plot(c_best, m_best, "*", markersize=16, color=OKABE[1], zorder=7)

    # Labels
    ax.annotate("Start  $\\boldsymbol{\\theta}_0$",
                xy=(c0, m0), xytext=(c0 - 0.55, m0 - 0.45),
                fontsize=9, color=OKABE[2], ha="right", va="top",
                arrowprops=dict(arrowstyle="->", color=OKABE[2], lw=0.8))
    ax.annotate("Best fit  $\\boldsymbol{\\theta}^*$",
                xy=(c_best, m_best), xytext=(c_best + 0.25, m_best + 0.45),
                fontsize=9, color=OKABE[1], ha="center", va="bottom",
...
def fig_goodness():
    ...
    for ax, (title, true_noise, reported_err) in zip(axes, scenarios):
        ...
        ax.errorbar(x, y, yerr=reported_err, fmt="o", color=OKABE[0],
                    markersize=4.0, capsize=3.5, capthick=1.2,
                    elinewidth=1.2, ecolor="#7f8c8d", zorder=2)
        ax.plot(x_fine, y_fit, color=OKABE[1], linewidth=1.4, zorder=3)
...
def fig_curvature():
    ...
    background = ax.contour(ints, slopes, SS, levels=15, cmap="Greys",
                           linewidths=0.3, alpha=0.5)

    # three highlighted contour levels: min, +1 (1σ), +4 (2σ)
    lvls = [chi2_min, chi2_min + 1, chi2_min + 4]
    cs = ax.contour(ints, slopes, SS, levels=lvls,
                    colors=[OKABE[2], OKABE[1], OKABE[3]],
                    linewidths=[1.0, 1.6, 1.0])
    ax.clabel(cs, fmt={lvls[0]: "$\\chi^2_{\\min}$",
                       lvls[1]: "$\\chi^2_{\\min}+1$",
                       lvls[2]: "$\\chi^2_{\\min}+4$"},
              fontsize=7.5)
    ...
    ax.annotate(
        ...
        fontsize=9, color=OKABE[1], ha="left",
    )
